...
def creer_taches(mois: int, date_lundi: str, historique_produits: list = None) -> list:
    ...
    tache_email_coach = Task(
        description=(
            ...
            "ordre précis : 1) appelle `uploader_dossier_drive` pour "
            "uploader le dossier complet de la semaine sur Google Drive ; "
            "2) appelle `envoyer_email_recap` avec date_semaine, le lien "
            "Drive obtenu à l'étape 1 (ou le message d'erreur si l'upload "
            "a échoué), et les noms exacts des deux produits vedettes de la "
            "semaine (produit1 et produit2, en minuscules) tels qu'ils "
            "apparaissent dans la section '## PRODUITS CHOISIS' du Chef "
            "Cuisinier Saisonnier."
        ),
        expected_output=(
